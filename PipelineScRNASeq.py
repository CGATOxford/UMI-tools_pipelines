...
@cluster_runnable
def plotERCCAccuracy(infiles, plotfiles):
    ''' merge the counts tables per sample, select the ERCC spike ins and
    plot the sd and mean'''

    plotfile, plotfile2 = plotfiles
    ...
    for infile in infiles:
        ...
        # hardcoded selection of ERCC spike-ins
        # these are the most abundant
        df = df.ix[["ERCC-00074", "ERCC-00002"]]
        df = df.transpose()
        df['method'] = method
    ...
    function(df){
      ...
      df$mean = (df$ERCC.00074 + df$ERCC.00002)/2
      df = df[order(df$mean),]
      df$bin = .bincode(seq(1/length(df[,1]), 1, 1/length(df[,1])),
                        seq(0,1,0.1))

      m_txt = element_text(size=16)
      l_txt = element_text(size=20)

      t = theme(axis.text.x=m_txt,axis.text.y=m_txt,
                axis.title.x=l_txt, axis.title.y=l_txt,
                legend.text=m_txt, legend.title=l_txt,
                panel.margin = unit(1, "lines"),
                legend.key.size=unit(2, "line"),
                legend.position="right",
                legend.direction="vertical")
      s_x = scale_x_continuous(breaks=seq(1,10,1))
      x = xlab("Expression decile")
      s = scale_colour_discrete(name="", guide=guide_legend(
                                title.position="top", title.hjust=0.5))

      p = ggplot(df, aes(bin, ERCC.00074/ERCC.00002)) +
      stat_summary(fun.y = function(x) sd(x)/mean(x),
                   geom="line", aes(colour=method)) +
      ylab("CV (sd/mean)") + s_x + x + s + theme_bw() + t

      ggsave("%(plotfile)s", width=10, height=5)


      p = ggplot(df, aes(bin, ERCC.00074/ERCC.00002)) +
      stat_summary(fun.y = mean, geom="line", aes(colour=method)) +
      ylab("Mean") + s_x + x + s + theme_bw() + t

      ggsave("%(plotfile2)s", width=10, height=5)
